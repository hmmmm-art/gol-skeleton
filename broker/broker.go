...
func (br *Broker) GolEngine(request stubs.Request, response *stubs.Response) error {
	...
	for turn = 0; turn < request.Params.Turns && !br.quit; {
		for i := 0; i < len(br.clients); i++ {
			...
			nodeReq := stubs.NodeArgs{
				WorldSlice: world[startRow : endRow+1],
				YStart:     startRow,
				YEnd:       endRow,
			...
			go func(i int, client *rpc.Client, req stubs.NodeArgs) {
				defer br.wait.Done()
				err := client.Call(stubs.WorkerGolEngine, req, nodeResp)
				if err != nil {
					response.State = nil
					response.CurrentTurn = 0
				}
				returnChans[i] <- nodeResp.WorldSlice
			}(i, br.clients[i], nodeReq)
            for br.pause {}
		}

		newWorld := make([][]byte, request.Params.ImageHeight)
		currentRow := 0
		for i := 0; i < len(returnChans); i++ {
			workerSlice := <-returnChans[i]
			for _, row := range workerSlice {
				newWorld[currentRow] = row
				currentRow++
			}
		}
		world = newWorld
...
func (br *Broker) CalculateAliveCells(request stubs.CountAliveRequest, response *stubs.AliveCellsResponse) (err error) {
	br.wait.Add(1)
	defer br.wait.Done()

	count := countAliveCells( br.world)
	br.lock.Lock()
