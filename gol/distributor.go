...
func distributor(p Params, c distributorChannels) {
	...
	go func() {
		for {
			key := <-c.keyPresses
			handleKeyPress(key, client, ticker, p, c)
		}
	}()
	...
	go func() {
		for {
			select {
			case <-ticker.C:
				req := stubs.CountAliveRequest{}
				resp := new(stubs.AliveCellsResponse)
				client.Call(stubs.CalculateAliveCells, req, resp)

				c.events <- AliveCellsCount{resp.Turn, resp.AliveCount}
			case <-tickerDone:
				return
			}
		}
